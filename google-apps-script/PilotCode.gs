// Google Apps Script for the PANORAMIC PILOT (?pilot-360).
//
// A separate copy of the main survey script, bound to a SEPARATE spreadsheet.
// That separation is the point: pilot data must never merge into the primary
// response dataset, and two different Sheets guarantee it in a way that a
// filter applied later does not.
//
// Differences from the main script: the tab is PilotResponses, and two extra
// columns are written — survey_version and median_seconds.
const SHEET_NAME = 'PilotResponses'
