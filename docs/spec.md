...
**Built 2026-09-10.** Route `#/diagnose`. Verify with `npm test` (280 tests, of which `test/zones.test.js`, `test/sandbox.test.js`, `test/presets.test.js`, `test/precedent.test.js`, `test/scenarios.test.js` and `test/isovist.test.js`'s closed-share block are this phase's) and `npm run validate:presets` (9/9). Decisions taken, and the three places the build departs from the paragraph above:
...
- **Deviation: a P9-only diagnostic metric, because compactness breaks.** Compactness (4πA/P²) collapses under sparse slender obstacles: a 250 mm post stops one ray while its neighbours run on a hundred metres, adding two long radial edges to the perimeter, and perimeter enters compactness squared. Measured here, a pergola takes 2.5% off isovist area and **53% off compactness** — an isoperimetric quotient doing what it does when a shape grows fine fringe, and useless for judging whether an intervention of posts or trunks did anything. Two metrics were added for this phase; one survives:
  ...
  - **Removed 2026-09-12: `solid_frontage_m`.** It was kept, computed but excluded from judgement, on the theory that a free field with no verdict does no harm. It reported how much built surface is *in view*, not how much was added — a kiosk that hides sixty metres of distant facade and shows eight metres of itself lowered it (kiosk −9.6%, market row −15.9%) while adding real surface, the exact case it existed to help with. A field that reads the wrong sign on its own motivating example belongs nowhere a reader might glance at it, disclaimer or not, so it was deleted from the metrics function, the preset expectations, the Tier B table, the validation harness and the on-page caveats rather than kept as a footnote.
  - **Added 2026-09-12: `closed_share`, occlusivity's own blind spot given the same treatment.** Occlusivity sums real METRES of unbroken wall — the same continuity rule as `solid_frontage` had, but restricted to continuous runs — and metres have a blind spot P9's sandbox is the first phase to expose: a near object and a far one can pass the identical continuity test and still disagree completely once converted to length, because a near surface is physically small no matter how much of the horizon it fills. Measured directly: standing among four pavilions close enough to surround a person drops occlusivity by two thirds (499 m → 175 m) while raising the SHARE of the horizon that is unbroken wall (0.897 → 0.958). `closed_share` tallies exactly that share, using occlusivity's own continuity test — an edge counts only where consecutive rays land on the same building, or on two within the touching tolerance — with nothing else changed. Across the seven saved scenarios it disagrees in sign with occlusivity in five of seven. Unlike `solid_share` it is not saturated at this site (baseline 0.897, not 0.981), so unlike `solid_share` its magnitude is trustworthy, not only its sign; `test/presets.test.js` pins that distinction directly, asserting every preset tags the two oppositely on `diagnostic`. It reaches the same three surfaces solidity does — the Tier B before/after table, the preset validation harness (9/9 pass), the on-page methods disclosure — and, like the other two P9-only fields, never reaches a weight model and is never written to any corpus file.
  - Neither reaches a weight model nor is written to any corpus data file. Zone assignment uses the four validated metrics alone, exactly as P6 does. `occlusivity_m` and `compactness` are untouched for P1–P8; no corpus value was recomputed for any earlier phase.
...
- **Added 2026-09-10: four named design scenarios, and the one result that reverses a claim made above.** `src/data/scenarios.json` ships seven worked scenarios rather than one. Six are stated design propositions and the seventh, *Four Types, Two Halves* (renamed 2026-09-12 from the placeholder `test`), is a reachability demonstration rather than an urban proposal — it asks whether a balanced, north/south-symmetric zone composition is attainable at all, and answers yes at 25.6 / 5.3 / 24.0 / 45.0 % with the halves 3.5 % apart. Of the design propositions, four were added first — *Rooms in the Square* (four 12 m pavilions staggered across the centre), *Market Hall* (one 60 × 18 m hall on the market axis with two kiosks), *Colonnaded Edge* (three recessed arcades plus one added colonnade), *Shaded Grove* (two tree rows, a 30 × 12 m canopy on 600 mm piers, a raised step platform) — each saved with the selection and target type it was drawn against so its diagnosis is reproducible rather than remembered. Two things they establish that the earlier single scenario could not:
...
- **Added 2026-09-10: two further scenarios, and the control that qualifies "every additive move travels the same way".** *Bridging the Level Break* treats the square's existing transport accesses as architecture — three 14 × 10 m pavilions at 6–7 m flanking them, raised steps on the square-facing side, one 20 m screen wall closing the gap between two. The accesses were read off the model rather than assumed: three 3.2 m structures stand inside the plaza, and each punches a matching hole in the standable lattice, so the fragmentation the scenario addresses is visible in the data before anything is drawn. *Zeil Terminus* closes the axis instead: one 20 × 20 m landmark at 15 m at the eastern end — the register puts the Zeil's centre 417 m due west, so the square is its terminus and the gesture faces west — with two 35 m colonnades compressing the approach.
  - **Position decides compactness, not mass, and the control is a 28-point swing.** The bullet above states that every additive preset lowers area, compactness and occlusivity together and that only demolition or the recess reverses it. *Bridging* raises compactness **+9.7%** while adding three substantial masses. The same three pavilions — identical footprints, identical heights — moved into open ground away from the accesses give **−19.9%** instead of **+8.4%**. Consolidating mass against an obstruction that is already there absorbs it into one silhouette; the same mass in open ground adds a second independent notch to the isovist. The generalisation to keep is therefore about *isolated* additions, and the qualification is not a detail: it is the difference between an intervention that reads as structure and one that reads as clutter, and the instrument distinguishes them. The same control is the only route found to **"Open, regular"** — 0.3% of the plaza at baseline, 9.3% here, **0.0%** in the displaced control.
  - **Height moves enclosure and nothing else, now measured under control rather than reasoned.** Holding footprint and position identical and varying only height, a 20 × 20 m mass gives isovist area **−18.1% at both 8 m and 15 m**, compactness **+5.9% at both**, and enclosure **9.9% against 16.9%** — near the mass, **+47.5% against +67.4%** within 25 m. The claim that the other three metrics are pure plan geometry was previously asserted from the engine's construction; this is the controlled measurement of it. It converts, too: "Tight, strongly enclosed" reaches 24.3% at 8 m and 33.3% at 15 m.
  - **And the scenario destroys its own result, which is the more useful half.** The landmark ALONE leaves the best-balanced plaza in the whole set (19.5 / 3.5 / 33.3 / 43.7 % at 15 m). Adding the two colonnades meant to reinforce it takes compactness to **−75.7%** and flattens the square to 93.2% "Tight, irregular". Directional compression built from columns does not compress, it serrates — the same mechanism *Colonnaded Edge* found, reached from the opposite direction. Note also that *Zeil Terminus* scores the highest neighbour agreement of any scenario (96.2%) while being the least differentiated: coherence by homogenisation is not structure, and a single agreement statistic cannot tell the two apart. *Bridging* reads worse on it (80.3%) and is the more structured map — 17 distinct zone patches against *Rooms in the Square*'s 13, with its largest patch 406 points against 670, and 57% of its reclassified points within 30 m of an access against 46% of the plaza lying there, where *Rooms*' changes are exactly proportional to area (49% against 49%) and therefore scattered by the measurement.
...
- **A methods disclosure at the foot of the page**, collapsed like P6's, P7's and P8's, carrying every deviation above, the three metric caveats, what each tier cannot tell you, the two counter-intuitive findings and the zone-dominance finding, and the live P8 numbers. A caveat recorded in a commit message is not disclosed.
  - **Added 2026-09-12: a worked example beside each of the three P9-only metrics.** Each is a measured case from this plaza with the current engine, set apart from the prose and flagging the row the paragraph is warning about, because a claim that a metric misleads is only worth the case that demonstrates it. *Solidity:* one colonnade with every slider at its minimum — four 0.6 m posts, 1.44 m² of blocking footprint in a 6,000 m² square — moves area −3.4% and solidity −3.1% while compactness reads **−31.8%**, and compactness is the one the typology reads, which is why those four posts move 'Open, facade-rich' from 68.0% to 30.6%. *Closed share:* standing among four 12 × 12 m pavilions 14 m away, occlusivity falls 499 m → 175 m while closed share rises 0.917 → 0.958 and enclosure rises 112% — the same continuity rule reaching opposite conclusions, with enclosure and area settling which one matches standing there. *Solid share:* at that same vantage only **7 of 360 rays** met open sky before anything was built, so 1.9% of the horizon is the entire range the measure can ever move at this site, whatever is drawn.
  - **Fixed 2026-09-12: Tier A rendered nothing at all, heading included, until an area was selected AND an intended type chosen.** A reader arriving at the page met a section headed 'Tier B' with no Tier A anywhere above it, which reads as a missing section rather than an empty one. The gate moved from around the section to inside it: the heading and its framing now render whenever the page has data, with an empty state naming the two things needed — the same answer the Tier B effect table already gives when nothing is selected.
